    ...
    <div className="relative bg-[#f4ece1] min-h-screen text-[#2a2a2a] overflow-x-hidden selection:bg-[#8b5a2b] selection:text-white">
      ...
      <section className="relative h-screen flex flex-col justify-center px-6 md:px-16 overflow-hidden">
        <div className="relative z-10">
          ...
          <motion.h1 initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.2, ease: [0.16, 1, 0.3, 1] }} className="text-[18vw] md:text-[14vw] font-black leading-[0.75] uppercase italic tracking-tighter">
            CAFÉ <br /> <motion.span initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.2, delay: 0.2, ease: [0.16, 1, 0.3, 1] }} className="text-[#8b5a2b] block md:inline">
              & DECO.
            </motion.span>
          </motion.h1>
        </div>
        ...
        <motion.img
          ...
          style={{ y: useTransform(smoothProgress, [0, 0.2], [0, -100]), rotate: 15 }}
      ...
      <section className="relative px-4 md:px-12 py-20 flex flex-col gap-40 md:gap-80">
        {PRODUCTOS.map((item, index) => (
          <SeccionProducto key={item.id} item={item} index={index} scrollProgress={smoothProgress} />
        ))}
      </section>
...
const SeccionProducto = ({ item, index, scrollProgress }) => {
  ...
  const xMove = useTransform(scrollProgress, [0, 1], isRight ? ["-20%", "10%"] : ["20%", "-10%"]);

  return (
    <div className={`relative flex flex-col ${isRight ? "md:items-end" : "md:items-start"} w-full`}>
      <motion.h2 style={{ x: xMove }} className="absolute top-1/2 -translate-y-1/2 text-[35vw] md:text-[25vw] font-black opacity-[0.07] italic uppercase pointer-events-none whitespace-nowrap z-0">
        {item.name}
      </motion.h2>

      <div className={`relative z-10 flex flex-col ${isRight ? "items-end text-right" : "items-start text-left"} max-w-2xl`}>
        <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex items-center gap-4 mb-2">
          <span className="text-[10px] font-black bg-[#8b5a2b] text-white px-2 py-1 uppercase">{item.id}</span>
          <span className="text-[10px] font-bold tracking-widest uppercase opacity-40">{item.deco}</span>
        </motion.div>

        <div className="relative group">
          <motion.div initial={{ opacity: 0, scale: 0.9, y: 30 }} whileInView={{ opacity: 1, scale: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8, ease: "easeOut" }} whileHover={{ scale: 1.02 }} className="relative w-[70vw] h-[70vw] md:w-[35vw] md:h-[35vw]">
            <img src={item.img} alt={item.name} className="w-full h-full object-contain drop-shadow-[0_50px_80px_rgba(0,0,0,0.15)] transition-transform duration-700" />
          </motion.div>
        </div>

        <div className="mt-[-70px] md:mt-[-50px] relative z-20">
          <motion.h3 initial={{ opacity: 0, x: isRight ? 30 : -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="text-5xl md:text-7xl font-black uppercase italic leading-none mb-2">
            {item.name}
          </motion.h3>
